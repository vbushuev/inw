...
int doCommand(sStep sstep){
    ...
    int ret = 0;
    dword iTimeout =0 , curTime = 0, di_data = 0, do_data = 0, commonTO=5000;//ict = 0
    long enc = 0, enc_s = 0;
    commonTO = gRegisters[0x25];
    di_data = readSignals2();DelayMs(16);
    // checks
    ...
    for(;;){
        ...
        if((di_data&H_06)){clearEncoder(0);enc_s=0;enc=0;DelayMs(16);} // нижний поршень в верхнем положении левый(0)
        if((di_data&H_12)){clearEncoder(1);enc_s=0;enc=0;DelayMs(16);} // нижний поршень в верхнем положении right

        if(sstep.wait.type==0 && (di_data&sstep.wait.value)){break;}
        else if(sstep.wait.type == 1 && (curTime >= sstep.wait.value) ){break;}
