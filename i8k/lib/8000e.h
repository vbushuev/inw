...
int GetRtsControlMode8000(int slot,int port);
...
extern long Com8000TxTimeout; // default=100(ms); [2013/11/26] add.
...
void InstallComInputData( int port,int (*)(unsigned char)); /* [2007/10/03] */
int IsComOutBufEmpty(int port); /* [2007/10/03] */
int SetComTimeout(int port,unsigned t); /* [2007/10/03] */
...
extern void (*InstallComInputData_[COM_PORT_NO+1])(int (*)(unsigned char)); /* [2007/10/03] */
extern int (*IsComOutBufEmpty_[COM_PORT_NO+1])(void); /* [2007/10/03] */
extern void (*SetComTimeout_[COM_PORT_NO+1])(unsigned t); /* [2007/10/03] */
...
/* 
[2004/07/27] add function for COM2 (for 8x1x/8x1x)
*/
void AddCom2Fun(void);
/*
 After call AddCom2Fun(), the functions for COM2 will be link into the .exe file.
 By default IsCom(2)/ReadCom(2)/... will NOT WORK for I-8x1x.
 After call AddCom2Fun(), IsCom(2)/ReadCom(2)/... will WORK for I-8x1x.
 
 IsCom_2()/ReadCom_2()/... always WORK for I-8x1x.
*/
...
[2004/11/16] Add
  ...
  After call InstallNewTimer(), must use _IsSystemKey() to instead of IsSystemKey(),
  use _GetSystemKey() to instead of GetSystemKey(),
  use _ClearSystemKey() to instead of ClearSystemKey().
*/

/*
[2005/02/14] add
For new C836 CPU card, COM3/4 share the same interrupt pin.
So for using COM3 and(or) COM4 just call InstallCom_34() once.
If the baud rate or the data format are not tha same for these two COM port,
just call SetBaudrate_3()/SetBaudrate_4() or SetDataFormat_3()/SetDataFormat_4() to set the 
baud rate or data format.
*/
int InstallCom_34(unsigned long baud, int data, int parity,int stop);
int RestoreCom_34(void);
...
/*
[2005/05/17] Add functions for S256/S512 on I-8000 back plate.
These functions are come from "S256L.LIB" 

For S256 : 2048 blocks, every block has 128 bytes. Total size is 256K bytes.
For S512 : 4096 blocks, every block has 128 bytes. Total size is 512K bytes.
*/
...
[input]
block:0-2047 for S256, 0-4095 for S512
offset:0-127
...
[return value]:
return the data sotred in sram.
*/
...
For 24LC16, size=2K bytes (I-8000/I-8000E-80)
EE_Type=16;
EE_BlockNo=8;
EE_PageSize=16; 
...
/*
[2010/07/13] Add functions for delay function to use Timer 0 or Timer 1.
*/
...
/*[2013/03/20] add*/
extern void (*Com8000GotData)(int slot,int port,unsigned char *buf,int size);
/*
user can install the callback function for Com8000(i-8114/8112/8144/8142).

void My_Com8000GotData(int slot,int port,unsigned char *buf,int size)
{
	// COM [slot][port] has got [size] bytes data stored in the buffer [buf].
}

void InstallCallBack(void)
{
	Com8000GotData=My_Com8000GotData;
}	
*/
